...
namespace Termgine {
  class DemoDnD {
    ...
    private static Scene SetupDragonTalkScene(){
      ...
      dragonFace.Position = display.Center - dragonFace.GetHeight()/2 + Vector2.Right * 20;
    ...
    private static Scene SetupWelcomeScene(){
      ...
      rightDragon.Position = welcomeWithBorder.Position - new Vector2(-welcomeWithBorder.GetWidth() - 1, rightDragon.GetHeight()/2);
      leftDragon.Position = new Vector2(rightDragon.Position.X - welcomeWithBorder.GetWidth() - leftDragon.GetWidth() - 1, rightDragon.Position.Y);
      var pressAnyKey = new Label("Press any key to start...");
